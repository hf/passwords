...
func testArgon2d(t *testing.T) {
	...
	hash, err := deriveKey(context.TODO(), argon2d, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32)
	if err != nil {
...
func testArgon2i(t *testing.T) {
	...
	hash, err := deriveKey(context.TODO(), argon2i, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32)
	if err != nil {
		t.Fatalf("failed to derive key: %v", err)
	}
	if !bytes.Equal(hash, want) {
...
func testArgon2id(t *testing.T) {
	...
	hash, err := deriveKey(context.TODO(), argon2id, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32)
	if err != nil {
		t.Fatalf("failed to derive key: %v", err)
	}
	if !bytes.Equal(hash, want) {
...
func TestVectors(t *testing.T) {
	...
	for i, v := range testVectors {
		...
		hash, err := deriveKey(context.TODO(), v.mode, password, salt, nil, nil, v.time, v.memory, v.threads, uint32(len(want)))
		if err != nil {
...
func benchmarkArgon2(mode int, time, memory uint32, threads uint8, keyLen uint32, b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		deriveKey(context.TODO(), mode, password, salt, nil, nil, time, memory, threads, keyLen)
	}
}
