...
func GenerateFromPassword(password []byte, cost int) ([]byte, error) {
	return GenerateFromPasswordWithContext(context.Background(), password, cost)
}

// GenerateFromPasswordWithContext returns the bcrypt hash of the password at the given
// cost. If the cost given is less than MinCost, the cost will be set to
// DefaultCost, instead. Use CompareHashAndPassword, as defined in this package,
// to compare the returned hashed password with its cleartext version.
...
func CompareHashAndPassword(hashedPassword, password []byte) error {
	return CompareHashAndPasswordWithContext(context.Background(), hashedPassword, password)
}
...
func bcrypt(ctx context.Context, password []byte, cost int, salt []byte) (retval []byte, reterr error) {
	done := make(chan struct{})

	internal.SubmitRequest(ctx, 1, func() {
		defer close(done)
		...
		retval = base64Encode(cipherData[:maxCryptedHashSize])
	})
