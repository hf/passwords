...
func TestBcryptingIsCorrect(t *testing.T) {
	...
	hash, err := bcrypt(context.TODO(), pass, 10, salt)
	if err != nil {
...
func TestVeryShortPasswords(t *testing.T) {
	key := []byte("k")
	salt := []byte("XajjQvNhvvRt5GSeFk1xFe")
	_, err := bcrypt(context.TODO(), key, 10, salt)
	if err != nil {
...
func TestTooLongPasswordsWork(t *testing.T) {
	...
	hash, err := bcrypt(context.TODO(), tooLongPass, 10, salt)
	if err != nil {
...
func TestCostValidationInHash(t *testing.T) {
	...
	for c := 0; c < MinCost; c++ {
		p, _ := newFromPassword(context.TODO(), pass, c)
		if p.cost != DefaultCost {
			t.Errorf("newFromPassword should default costs below %d to %d, but was %d", MinCost, DefaultCost, p.cost)
		}
	}

	p, _ := newFromPassword(context.TODO(), pass, 14)
	if p.cost != 14 {
	...
	_, err := newFromPassword(context.TODO(), pass, 32)
	if err == nil {
...
func TestCostReturnsWithLeadingZeroes(t *testing.T) {
	hp, _ := newFromPassword(context.TODO(), []byte("abcdefgh"), 7)
	cost := hp.Hash()[4:7]
	expected := []byte("07$")
